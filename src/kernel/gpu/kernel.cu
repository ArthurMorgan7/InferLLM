...
void llm_elemwise_broadcast_dim0_src1_compute_float(
        const float* src0, const float* src1, float* dst, uint32_t rows, uint32_t ncols, ElemMode mode, cudaHandle* handle) {
    cudaStream_t stream = handle->stream;
    const dim3 block_dims(512, 1, 1);   // 线程块大小
    const dim3 block_nums((ncols + 511) / 512, rows, 1);    // 线程网格大小
...
void llm_elemwise_compute_float_scale(float* src, float* dst, size_t len, float scale, cudaHandle* handle) {
    cudaStream_t stream = handle->stream;
    const dim3 block_dims(CUDA_NUM_THREADS, 1, 1);
    const dim3 block_nums((len + CUDA_NUM_THREADS - 1) / CUDA_NUM_THREADS, 1, 1);
    llm_elemwise_compute_float_scale_gpu
        <<<block_nums, block_dims, 0, stream>>>(src, dst, len, scale);
}
...
// CPU 调用接口
template <typename Function, typename... Args>
cudaError_t LaunchKernel(Function fun, cudaStream_t stream, int64_t n, float* ret, Args... args) {
...
__global__ void softmax_f32_cuda(const float* x, float* dst, const int cols) {
    const int row = blockDim.y * blockIdx.y + threadIdx.y;  // 线程对应的矩阵的行
    const int block_size = blockDim.x;  
    const int tid = threadIdx.x;    // 局部索引 1~32
    const float* src = x + row * cols;  // 行首元素的指针
    dst = dst + row * cols;             // 
...
#pragma unroll
    ...
    // Softmax 第二阶段：归一化输出（每个线程负责一部分数据）
    for (int col = tid; col < cols; col += block_size) {
        dst[col] /= sum;
    }
}
...
// CPU 上调用核函数的接口
void llm_softmax_compute_float(const float* src, float* dst, uint32_t len_row, uint32_t col, cudaHandle* handle) {
    cudaStream_t stream = handle->stream;
    const dim3 block_dims(kNumWaves, 1, 1); // 线程块  ：[32,1,1] [x,y,z]  [列,行，深]
    const dim3 block_nums(1, len_row, 1);   // 线程网格：[]
...
void llm_matmul_compute_float_float(
    ...
    CUBLAS_CHECK(cublasSgemm(cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, N, M, K, &alpha, src0, K, src1, K, &beta, dst, N));
    if (bias != nullptr) {
        llm_elemwise_broadcast_dim0_src1_compute_float(dst, bias, dst, M, N, ElemMode::Add, handle);
    }
}
